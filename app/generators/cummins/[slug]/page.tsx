...
// Node imports — allowed in server components
import fs from "fs";
...
export default function GeneratorSpecPage(props: { params: Promise<{ slug: string }> }) {
  ...
  // Try to read the PDF file from public/ and build a data: URI (base64).
  // If anything fails (file missing, LFS, too large), fall back to product.fullspec.
  let pdfHref = product.fullspec; // default fallback (static URL)
  try {
    // product.fullspec is like "/generators/cummins/cummins-c22d5-data-sheet.pdf"
    const relativePath = product.fullspec.replace(/^\//, ""); // remove the leading slash
    const filePath = path.resolve(process.cwd(), "public", relativePath);
    // Read as Buffer and convert to base64
    const fileBase64 = fs.readFileSync(filePath).toString("base64");
    pdfHref = `data:application/pdf;base64,${fileBase64}`;
    // NOTE: for very large PDFs you may not want to embed as a data URI.
  } catch (err) {
    // Keep fallback; this will let the existing link work (and produce a normal request to /generators/...)
    // Useful diagnostic in server logs:
    // console.warn(`[PDF read fallback] Could not read ${product.fullspec}:`, err);
  }
  ...
  return (
    <>
      {/* Inject schema data */}
      <Script type="application/ld+json" id="org-schema" dangerouslySetInnerHTML={{ __html: JSON.stringify(orgSchema) }} />
      <Script type="application/ld+json" id="product-schema" dangerouslySetInnerHTML={{ __html: JSON.stringify(productSchema) }} />
      ...
      <section>
        ...
        >
          <div className="container h-full flex items-end pb-4">
            <h1 className="text-[var(--background)] text-xl md:text-2xl">{product.metaTitle}</h1>
          </div>
        </div>
        ...
        <div className="container py-12 flex flex-col md:flex-row mt-3 gap-6 items-start">
          ...
          <div className="w-full md:w-[50%]">
            <Image src={product.image} alt={product.title} width={800} height={600} className="rounded-xl shadow-md w-full object-cover" />
          </div>

          {/* Content */}
          <div className="w-full space-y-6">
            <h1 className="text-[var(--foreground)] text-xl md:text-2xl lg:text-4xl">{product.title}</h1>
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 text-base text-[var(--foreground)] pb-4">
              {[
                ...
                <div key={i} className="flex justify-between items-center border-b border-gray-200 last:border-0 pb-2">
                  <dt>{spec.label}:</dt>
                  <dd>{spec.value}</dd>
                </div>
              ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button className="w-full lg:w-1/2 px-6 md:px-8 lg:px-10 py-3 text-left btn-primary shine-effect cursor-pointer">Enquiry</button>

              {/* Use a normal anchor so data: URIs / external links open correctly */}
              <a
                href={pdfHref}
                target="_blank"
        ...
        <div className="container mt-16 text-[var(--foreground)] max-w-4xl space-y-14">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {product.specs.map((section, i) => (
              <section key={i}>
                <h2 className="text-xl font-base text-[var(--foreground)] mb-4 border-b pb-2">{section.group}</h2>
                <div className="grid grid-cols-1 gap-4">
                  {section.items.map((item, idx) => <SpecItem key={idx} label={item.label} value={item.value} />)}
                </div>
              </section>
            ))}
          </div>
