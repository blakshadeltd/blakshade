...
const CumminsGeneratorsClient: React.FC<CumminsGeneratorsClientProps> = ({ searchParams }) => {
  ...
  const [selectedBrand, setSelectedBrand] = useState<string>(searchParams.brand || "Cummins");
  ...
  const allGenerators = useMemo(() => [...cummins], []);
        ...
        <main className="w-full lg:w-3/4">
          <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
            <div className="text-gray-600">
              Showing all <span>{totalItems}</span> Cummins generators
            </div>
          ...
          {filteredAll.length === 0 && (
            <div className="text-center py-12">
              <p className="text-lg text-gray-600">No Cummins generators found matching your filters.</p>
            </div>
          )}
        </main>
      </div>
