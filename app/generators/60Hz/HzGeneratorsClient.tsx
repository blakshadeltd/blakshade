...
interface HzGeneratorsClientProps {
  searchParams: SearchParams;
}

const HzGeneratorsClient: React.FC<HzGeneratorsClientProps> = ({ searchParams }) => {
  ...
  const [selectedFrequency, setSelectedFrequency] = useState<string>(searchParams.frequency || "60Hz");
    ...
    <section>
      ...
      >
        <div className="container h-full flex items-end pb-4">
          <h1 className="text-[var(--background)] text-2xl md:text-4xl">60Hz Generators</h1>
        </div>
      </div>
        ...
        <main className="w-full lg:w-3/4">
          <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
            <div className="text-gray-600">
              Showing all <span>{totalItems}</span> 60Hz generators
            </div>
          ...
          {filteredAll.length === 0 && (
            <div className="text-center py-12">
              <p className="text-lg text-gray-600">No 60Hz generators found matching your filters.</p>
            </div>
          )}
        </main>
      </div>
...
export default HzGeneratorsClient;